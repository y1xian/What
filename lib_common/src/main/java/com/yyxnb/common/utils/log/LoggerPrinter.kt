...
object LoggerPrinter : IPrinter {
    ...
    private fun logContent(priority: Int, chunk: String) {
        val lines = chunk!!.split(LINE_SEPARATOR).toTypedArray()
        for (line in lines) {
    ...
    private fun getStackInfo(priority: Int) {
        logChunk(priority, HORIZONTAL_DOUBLE_LINE.toString() + "[Thread] → " + Thread.currentThread().name)
