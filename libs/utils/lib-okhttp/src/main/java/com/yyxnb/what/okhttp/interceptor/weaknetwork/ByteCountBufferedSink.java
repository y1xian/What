...
public class ByteCountBufferedSink implements BufferedSink {
    ...
    public BufferedSink write(@NotNull ByteString byteString, int offset, int byteCount) throws IOException {
        ...
        //计算出要写入的次数
        long count = (long) Math.ceil((double) byteString.size() / mByteCount);
        for (int i = 0; i < count; i++) {
            //让每次写入的字节数精确到mByteCount 分多次写入
            long newOffset = i * mByteCount;
            long writeByteCount = Math.min(mByteCount, byteString.size() - newOffset);
            buffer().write(byteString, (int) newOffset, (int) writeByteCount);
            emitCompleteSegments();
        }
        return this;
    }
