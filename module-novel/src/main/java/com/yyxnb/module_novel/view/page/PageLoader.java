...
public abstract class PageLoader {
    ...
    private static final int EXTRA_TEXT_SIZE = 10;

    // 当前章节列表
    protected List<TxtChapter> mChapterList;
    // 一共有多少章
    protected List<BookChapterBean> bookChapters;
    // 书本对象
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = DpUtils.dp2px(mContext, DEFAULT_MARGIN_WIDTH);
        mMarginHeight = DpUtils.dp2px(mContext, DEFAULT_MARGIN_HEIGHT);
        // 配置文字有关的参数
    ...
    private void setUpTextParams(int textSize) {
        // 文字大小
        mTextSize2 = textSize;
        mTextSize = textSize + DpUtils.sp2px(mContext, EXTRA_TEXT_SIZE);
        mTitleSize = mTextSize + DpUtils.sp2px(mContext, EXTRA_TITLE_SIZE);
        // 行间距(大小为字体的一半)
    ...
    private void initPaint() {
        ...
        mTipPaint.setTextSize(DpUtils.sp2px(mContext, DEFAULT_TIP_SIZE)); // Tip默认的字体大小
    ...
    public void setTextSize(int textSize) {
        ...
        mTitlePaint.setTextSize(mTitleSize);
        // 存储文字大小
        mSettingManager.setTextSize(mTextSize2);
        // 取消缓存
        mPrePageList = null;
        mNextPageList = null;
...
//        }
    ...
    public void saveRecord() {
        ...
        if (mChapterList.isEmpty() || bookInfoBean == null) {
            return;
        }
...
//            } else {
//                DBHelper.getInstance().getDb().bookRecordDao().update(mBookRecord);
//            }
...
//            LogUtils.INSTANCE.e(DBHelper.getInstance().getDb().bookRecordDao().getAllByIds(mCollBook.getBookId()).toString());
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = DpUtils.dp2px(mContext, 3);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            if (!mChapterList.isEmpty()) {
                ...
                float tipTop = tipMarginHeight - mTipPaint.getFontMetrics().top + 20;
                //根据状态不一样，数据不一样
                if (mStatus != STATUS_FINISH) {
//                    if (isChapterListPrepare) {
//                        float x = mDisplayWidth - (ScreenUtils.spToPx(mCurPage.title.length() * DEFAULT_TIP_SIZE));
//                        canvas.drawText(mChapterList.get(mCurChapterPos).getTitle()
//                                , x, tipTop, mTipPaint);
//                        canvas.drawText(bookName, mMarginWidth, tipTop, mTipPaint);
//                    }
                } else {
                    float x = mDisplayWidth - (DpUtils.sp2px(mContext, mCurPage.title.length() * DEFAULT_TIP_SIZE));
                    canvas.drawText(mCurPage.title, x - 20, tipTop, mTipPaint);
            ...
            //擦除区域
            mBgPaint.setColor(mBgColor);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + DpUtils.dp2px(mContext, 2), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
        ...
        int polarHeight = DpUtils.dp2px(mContext, 6);
        int polarWidth = DpUtils.dp2px(mContext, 2);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                polarTop + polarHeight - DpUtils.dp2px(mContext, 2));
        ...
        int outFrameBottom = visibleBottom - DpUtils.dp2px(mContext, 2);
