...
object CacheManager {
    ...
    fun <T> delete(key: String, body: T) {
        ...
        CacheDatabase.instance.cacheDao().deleteItem(cache)
    }
    ...
    fun <T> save(key: String, body: T) {
        ...
        cache.key = key
        cache.data = toByteArray(body)
        CacheDatabase.instance.cacheDao().insertItem(cache)
    }

    fun getCache(key: String): Any? {
        val cache = CacheDatabase.instance.cacheDao().getCache(key)
