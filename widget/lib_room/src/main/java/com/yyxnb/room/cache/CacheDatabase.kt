 ...
 */
@Database(entities = [Cache::class], version = 1, exportSchema = false) //数据读取、存储时数据转换器,比如将写入时将Date转换成Long存储，读取时把Long转换Date返回
                    ...
                    synchronized(CacheDatabase::class.java) {
                        if (INSTANCE == null) {
                            INSTANCE =  //创建一个内存数据库
                                    ...
                                    Room.databaseBuilder(AppUtils.app, CacheDatabase::class.java, "net_cache.db")
                                            //是否允许在主线程进行查询
